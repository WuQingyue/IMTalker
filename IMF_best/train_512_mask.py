...
from dataset_512 import VFHQ_mask, CombinedDataset
...
from networks.model import IMFModel
...
class IMFSystem(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.args.use_arcface:
            loss_face = self.face_loss_fn(pred, real)
    ...
    def validation_step(self, batch, batch_idx):
        # 生成双向重建
        pred = self.gen(batch["image_1_256"], batch["image_0_256"])
        recon = self.gen(batch["image_0_256"], batch["image_0_256"])            

        # 计算损失
        pred_l1 = F.l1_loss(pred, batch["image_1_512"])
        pred_vgg_all, pred_vgg_face = self.criterion_vgg(pred, batch["image_1_512"], batch["mask_eye_1"]+batch["mask_mouth_1"])
        recon_l1 = F.l1_loss(recon, batch["image_0_512"])
        recon_vgg_all, recon_vgg_face = self.criterion_vgg(recon, batch["image_0_512"], batch["mask_eye_0"]+batch["mask_mouth_0"])

        # 汇总
        loss_pred = self.hparams.loss_l1 * pred_l1 + self.hparams.loss_vgg_all * pred_vgg_all + self.hparams.loss_vgg_face * pred_vgg_face
        loss_recon = self.hparams.loss_l1 * recon_l1 + self.hparams.loss_vgg_all * recon_vgg_all + self.hparams.loss_vgg_face * recon_vgg_face
        val_loss = (loss_pred + loss_recon) / 2

        if self.args.use_arcface:
            loss_face = self.face_loss_fn(pred, batch["image_1_512"])
        ...
        img_list = [batch["image_0_512"], batch["image_1_512"], pred, recon, \
                    (batch["mask_eye_0"] + batch["mask_mouth_0"]), (batch["mask_eye_1"] + batch["mask_mouth_1"])]
        for name, img in zip(
            name_list,
            img_list
        ):
    ...
    def load_ckpt(self, ckpt_path):
        ...
        # 加载 generator 的权重
        gen_state_dict = {k.replace("gen.", ""): v for k, v in state_dict.items() if k.startswith("gen.")}
        missing_gen, unexpected_gen = self.gen.load_state_dict(gen_state_dict, strict=False)
        print(f"[INFO] Loaded generator. Missing keys: {missing_gen}, Unexpected keys: {unexpected_gen}")

        # 加载 discriminator 的权重（如果存在）
        if self.args.use_gan and hasattr(self, "disc"):
            disc_state_dict = {k.replace("disc.", ""): v for k, v in state_dict.items() if k.startswith("disc.")}
            if disc_state_dict:
                missing_disc, unexpected_disc = self.disc.load_state_dict(disc_state_dict, strict=False)
                print(f"[INFO] Loaded discriminator. Missing keys: {missing_disc}, Unexpected keys: {unexpected_disc}")
            else:
...
class CombinedDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        self.train_dataset = CombinedDataset((VFHQ_mask('train', ["vfhq"]), VFHQ_mask('train', ["vox2"]),VFHQ_mask('train', ["multitalk"])),(0.4,0.3,0.3))
        self.val_dataset = CombinedDataset((VFHQ_mask('test', ["vfhq"]), VFHQ_mask('test', ["vox2"]),VFHQ_mask('test', ["multitalk"])),(0.4,0.3,0.3))

...
class IMFModel(nn.Module):
    ...
    def forward(self, x_current, x_reference):
        ...
        # --- 核心修正：从最粗糙的层级开始，反向循环 ---
        # reversed(range(num_levels)) 会生成 5, 4, 3, 2, 1, 0
        for i in reversed(range(num_levels)):
            
            # 使用统一的索引 i 来获取所有对应的特征和模块
            query_feat = m_c[i]
            key_feat = m_r[i]
            value_feat = f_r[i]
            attention_block = self.implicit_motion_alignment[i]

            # 检查当前层级是否是粗粒度层（使用标准全注意力）
            # is_standard_attention 应该是在工厂 __init__ 中设置的属性
            if attention_block.is_standard_attention:
                # 这是粗粒度层。如果 attention_map 是 None，说明这是第一层（最粗糙的8x8层）。
                # 它会计算并返回新的 attention_map。
                # 即便不是第一层粗粒度层（如16x16），它也会计算自己的全注意力图，并覆盖掉之前的。
                aligned_feature, attention_map = attention_block(query_feat, key_feat, value_feat)
                aligned_features[i] = aligned_feature
            else:
                # 这是精细粒度层，必须接收来自上一层（更粗糙层）的 attention_map
                if attention_map is None:
                    raise RuntimeError("精细粒度层没有收到来自粗粒度层的 attention map 指导。")
                
                # 调用引导式稀疏注意力，它只需要一个返回值
                aligned_feature = attention_block(query_feat, key_feat, value_feat, attn=attention_map)
                aligned_features[i] = aligned_feature
                
        
        # aligned_features 列表现在已按 fine -> coarse 的顺序正确填充

...
from models.float.CFMT import ConditionFMT
...
class System(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # 3. 准备输入 (逻辑不变)
        base_context = { "m_now": noised_motion, "a_now": batch["a_now"], "m_prev": batch["m_prev"], "a_prev": batch["a_prev"] }

        # 4. 预测 (逻辑不变)
        batch_anchor = {**base_context, "m_ref": m_ref_anchor}
        pred_flow_anchor = self.model(batch_anchor, t=times)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        m_now = batch["m_now"]; m_ref_anchor = batch["m_ref"]
...
class DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        self.train_dataset = AudioMotionDataset(opt=self.opt, start=100, end=-1)
        self.val_dataset = AudioMotionDataset(opt=self.opt, start=0, end=100)

...
from models.float.FMT_gaze_smirk_adapter import FlowMatchingTransformer, AdapterTransformer
...
class ConditionFMT(BaseModel):
    def __init__(self, opt):
        ...
        self.embedding = nn.Parameter(torch.randn(opt.dim_motion))
        # Flow Matching Transformer
        self.fmt = FlowMatchingTransformer(opt)
        self.adapter = AdapterTransformer(opt)
        self.odeint_kwargs = {
            'atol': opt.ode_atol,
            'rtol': opt.ode_rtol,
    ...
    def forward(self, batch, t):
        ...
        bs = x.size(0)
        gaze_var = torch.var(gaze, dim=1)
        pose_var = torch.var(pose, dim=1)
        cam_var  = torch.var(cam, dim=1)
        var =torch.concat((gaze_var, pose_var, cam_var),dim=-1)
        # reshape 多层音频或条件特征
        ...
        embedding_with_dims = self.embedding.unsqueeze(0).unsqueeze(0)
        embedding = embedding_with_dims.expand(bs, seq_len, -1)
        pred_condition = self.adapter(
            var,
            embedding,
            a,
            prev_x,
        ...
        # 返回去掉 prev 部分的预测
        return pred[:, self.num_prev_frames:, ...], pred_condition

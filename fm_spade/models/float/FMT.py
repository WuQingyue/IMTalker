...
class FlowMatchingTransformer(BaseModel):
	...
	def __init__(self, opt) -> None:
		...
		alignment_mask = enc_dec_mask(self.num_total_frames, self.num_total_frames, 1, expansion=opt.attention_window)
	...
	def forward(self, t, x, a,  prev_x, prev_a, ref_x, train = True, **kwargs) -> torch.Tensor:
		"""
		...
		#ref_x = self.sequence_embedder(ref_x.unsqueeze(1), dropout_prob = self.opt.ref_dropout_prob, train=train)
		...
		#c = torch.cat([a, pose, gaze], dim=-1)
		
		#c = torch.cat([ref_x, a], dim=-1)
	...
	def forward_with_cfv(self, t, x, a,  prev_x, prev_a, ref_x, a_cfg_scale=1.0, **kwargs) -> torch.Tensor:
		if a_cfg_scale != 1.0 :
			...
			null_prev_a = torch.zeros_like(prev_a)
			...
			prev_a_cat = torch.cat([null_prev_a, null_prev_a], dim=0)

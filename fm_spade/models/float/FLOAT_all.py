...
from models.float.ap import Audio_projection
...
class FLOAT(BaseModel):
	def __init__(self, opt):
		...
		self.motion_autoencoder = IMFModel()
	...
	def encode_image_into_latent(self, x: torch.Tensor) -> list:
		f = self.motion_autoencoder.dense_feature_encoder(x)
		t = self.motion_autoencoder.latent_token_encoder(x)
		return f, t
	
	def decode_latent_into_motion(self, t: torch.Tensor):
		return self.motion_autoencoder.latent_token_decoder(t)
	...
	def decode_latent_into_image(self, f_r: torch.Tensor , t_r: torch.Tensor, t_c: torch.Tensor) -> dict:
		...
		for t in range(T):
			...
			img = self.motion_autoencoder.ima(m_c, m_r, f_r)
			d_hat.append(img)
	...
	def inference(
		...
		s, a = data['s'], data['a']
		f_r, t_r = self.encode_image_into_latent(s.to(self.opt.rank))
		data["ref_x"] = t_r
		# set conditions
		if a_cfg_scale is None: a_cfg_scale = self.opt.a_cfg_scale

		sample = self.sample(data, a_cfg_scale = a_cfg_scale,  nfe = nfe, seed = seed)

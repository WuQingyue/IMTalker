...
class TransformerBlock(nn.Module):
    def __init__(self, dim, heads, dim_spatial, mlp_dim):
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, dim_spatial, dim))
    ...
    def forward(self, x):
        ...
        pos = self.pos_embedding.expand(-1, -1, -1)
        pos = pos.permute(1, 0, 2).expand(seq_len, B, C)
        x_reshaped = x_reshaped + pos
        ...
        att_output, attn_weights = self.attention(x_norm ,
                                                  x_norm ,
                                                  x_norm )
...
if __name__ == "__main__":
    ...
    model = ImplicitMotionAlignment(feature_dim, motion_dim, (H,W), heads, dim_head, mlp_dim).to(device)

...
from utils.modules import FeatResBlock,UpConvResBlock,DownConvResBlock,ResBlock
from utils.lia_resblocks import StyledConv,EqualConv2d,EqualLinear
from utils.vit import ImplicitMotionAlignment
from utils.viz import FeatureMapVisualizer
import math
visualizer = FeatureMapVisualizer(save_dir="vis_output", mode="channel")
...
class FrameDecoder(nn.Module):
    def __init__(self, feature_dims, spatial_dims, depth=2):
        """
        一个健壮的、支持双特征流融合的解码器。
        ...
        # 1. 用于处理最深层融合的初始模块
        #    输入: src最深层 + align最深层
        self.conv_in = ConvResBlock(feature_dims[0] + feature_dims[0], feature_dims[0])
        ...
        self.resblocks = nn.ModuleList([
            ConvResBlock(feature_dims[i+1] * 3, feature_dims[i+1]) for i in range(len(feature_dims) - 1)
        ])
        ...
        self.transformer_blocks = nn.ModuleList([
            # 最深层 (循环外处理)
            nn.Sequential(*[TransformerBlock(dim=feature_dims[0], heads=8, dim_spatial=spatial_dims[0]**2) for _ in range(depth)]),
            # 后续层 (循环内处理)
            *[nn.Sequential(*[TransformerBlock(dim=feature_dims[i+1], heads=8, dim_spatial=spatial_dims[i+1]**2) for _ in range(depth)]) for i in range(len(feature_dims) - 1)]
        ])

        # 5. 最终输出层
        self.final_conv = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
            nn.Conv2d(feature_dims[-1], 64, kernel_size=3, padding=1),
            nn.Conv2d(64, 3, kernel_size=3, padding=1),
            nn.Tanh() # 使用 Tanh (-1, 1) 通常比 Sigmoid (0, 1) 效果更好，取决于数据归一化
        )

    def forward(self, features_src, features_align):
        """
        Args:
        ...
        # 1. 处理最深层 (解码起点)
        #    融合src和align的最深层特征
        x = torch.cat([features_src[-1], features_align[-1]], dim=1)
        x = self.conv_in(x)
        ...
        for i in range(len(self.upconv_blocks)):
            ...
            src_skip = features_src[-(i + 2)]
            align_skip = features_align[-(i + 2)]
            x = torch.cat([x, src_skip, align_skip], dim=1)
